...
class List {
  ...
  parse(token, inline, inlineText, tokFunc, nextFunc) {
    ...
    if (type === this.startTokenTypes[1]) { // list item
      ...
      while (nextFunc().type !== this.endTokenTypes[1]) {
        body += !loose && type === 'text' ? inline.output(text) : tokFunc();
      }
